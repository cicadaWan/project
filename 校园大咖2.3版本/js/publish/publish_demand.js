...
$(".left_choose").click(function() {
	...
	if($(this).hasClass("publishConsult_border")) {
		...
		moveArr1.splice(0, moveArr1.length)
	}
	...
	$(a).unbind('click').click(function() {
		...
		if(str === '.left_choose') {
			...
			if($(this).hasClass("publishConsult_border")) {
				...
				//
				$(e).eq(index).find("span:nth-of-type(2)").addClass("choose_color").removeClass("color");
				...
				//
				$(e).eq(index).find("span:nth-of-type(2)").removeClass("choose_color").addClass("color");
				for(i = 0; i < b; i++) {
			...
			if($(this).hasClass("publishConsult_border")) {
				...
				if(liOnoff) {
					...
					//
					$(selectQuery).eq(indexNow).find(".school_name").removeClass("choose_color").addClass("color")
...
$(".choose_school").click(function() {
	...
	////左选
	touch(arr1, ".school_Content .publishConsult_Left", $(".school_Content .box_left ul li").length, $(".school_Content .school_Box .box_left ul li"), ".school_Content .left_choose", ".publishConsult_Left", ".school_Content .school_box ul");
	////右选
	touch(arr1, ".school_Content .publishConsult_left", $(".school_Content .school_box ul li").length, $(".school_Content .school_Box .box_right ul li"), ".school_Content .right_choose", ".publishConsult_left")
...
//城市选择
sure(".ensure_city", ".city_content .school_box li .publishConsult_left", cityName, ".city_content", ".school_city", arr2, "个城市");
//学校选择
sure(".ensure_school", ".school_Content .school_box li .publishConsult_left", schoolName, ".school_Content", ".choose_school", arr1, "所高校")
//j为点击确定按钮，k为二级选项下的所有左边图标，l为schoolName或者cityName，m为整个弹窗类名，n为选择活动城市或者活动院校,Arr为数组,name为名字
function sure(j, k, l, m, n, Arr, name) {
	$(j).click(function() {
		
		//将元素全部清空
		$(n).html("");
	Arr=type==".city_content"?arr2:arr1;
        if(Arr.length>1){
        	l = Arr[0] + "等" + Arr.length + name
        }else{
        	l = Arr[0]
        }
